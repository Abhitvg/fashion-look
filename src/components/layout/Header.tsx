  ...
  return (
    <header className={`fixed w-full top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-atelier/90 backdrop-blur-md border-b border-atelier-soft py-4' : 'bg-transparent py-6'}`}>
      <div className="container mx-auto px-4 md:px-8 flex justify-between items-center">
        ...
        {/* Mobile Toggle */}
        <button className="md:hidden text-foreground hover:text-gold transition-colors" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-atelier border-b border-atelier-soft flex flex-col p-6 space-y-6 md:hidden">
